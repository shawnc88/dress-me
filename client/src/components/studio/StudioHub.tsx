 ...
 * mode carousel, one primary action). Lives in two places: the /studio route,
 * and the right-hand panel of the home pager (swipe the feed right-to-left).
...
export function StudioHub({ onClose }: { onClose?: () => void }) {
  const router = useRouter();
  const [active, setActive] = useState(0);
  const mode = MODES[active];

  return (
    <div className="absolute inset-0 celebration-canvas grain bg-ink-950 flex flex-col safe-area-pt safe-area-pb">
