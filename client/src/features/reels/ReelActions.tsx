...
export function ReelActions({ liked, likesCount, commentsCount, sharesCount, onLike, onComment, onShare }: ReelActionsProps) {
  return (
    <div className="flex flex-col items-center gap-5">
      {/* Like — min 44px tap target, glimmer on touch */}
      <motion.button
        whileTap={{ scale: 1.4 }}
        transition={{ type: 'spring', stiffness: 500, damping: 15 }}
        onClick={onLike}
        className="flex flex-col items-center gap-1 min-w-[44px] min-h-[44px] justify-center glimmer rounded-2xl overflow-hidden"
        aria-label="Like"
      >
        <motion.div
        ...
        >
          <Heart className={`w-7 h-7 ${liked ? 'fill-live text-live drop-shadow-[0_0_8px_rgba(255,48,64,0.7)]' : 'text-white'}`} />
        </motion.div>
        <motion.span
          key={likesCount}
